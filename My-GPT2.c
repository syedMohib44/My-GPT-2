...
typedef struct
{
    float *m; // First moment vector (mean of gradients)
    float *v; // Second moment vector (variance of gradients)
    int t;    // Time step (iteration number)
} AdamState;

void residual_layer(float *input1, float *input2, float *output, int size)
{
...
void matrix_mul(const float *inp, const float *weight, float *bias, float *out, int m, int k, int n, int B)
{
...
// Transposing and matrix mul is a bit processing intensive trying napkins first
// bad naming of vars but dout is actually the gradient coming into the block not going 'out' of the block
void matrix_mul_backprop(const float *inp, const float *weight, const float *dout, float *din, float *dweight, float *dbias, int m, int k, int n, int B)
{
#pragma omp parallel for collapse(2)
    for (int i = 0; i < B; i++)
    {
...
// d = derivded dinputs = dervidedinputs
// B, T, C
// mean = sum(x, dim = -1)/c
// var = (x - mean)^2
...
{
    // b, t, c = b, t, 4c
    matrix_mul(input, mlp->c_fc_weight, mlp->c_fc_bias, output1, T, C, 4 * C, B); // output of first layer is with gelu input
    GELU(output1, gelu_out1, B * T * 4 * C);
    //b, t, 4c = b, t, c
    matrix_mul(gelu_out1, mlp->c_proj_weight, mlp->c_proj_bias, output2, T, 4 * C, C, B);
...
void mlp_backprop(float *input, float *dinput, struct Input_Output *io, struct MLP *mlp, struct dMlp *dmlp, int B, int T, int C)
{
    ...
    matrix_mul_backprop(input, mlp->c_fc_weight, io->dmlp_l1_out, dinput, dmlp->d_fc_weight, dmlp->d_fc_bias, T, C, 4 * C, B);
}
...
void causal_attention(float *input, float *output, struct AttentionBlock *attn, struct Input_Output *io, int B, int T, int C, int num_heads)
{
    ...
    matrix_mul(input, attn->attn_weight, attn->attn_bias, io->qkv, T, C, 3 * C, B);
#pragma omp parallel for collapse(3)
    for (int i = 0; i < B; i++)
    {
        for (int n = 0; n < N; n++)
        {
            for (int j = 0; j < T; j++)
            {
                ...
                // we have calculated scores for a single row in the T x T self att matrix, now we apply softmax and matrix_mul with V
    ...
    matrix_mul(io->causal_attention, attn->proj_weight, attn->proj_bias, output, T, C, C, B);
...
void causal_attention_backprop(struct AttentionBlock *attn, struct dAttentionBlock *dAtt, struct Input_Output *io, int B, int T, int C, int num_heads)
{
    ...
    int H = C / N;
    // stage 3
    matrix_mul_backprop(io->causal_attention, attn->proj_weight, io->datt_out, dattn, dAtt->dproj_weight, dAtt->dproj_bias, T, C, C, B);
    //stage 2
...
#pragma omp parallel for collapse(3)
    ...
    matrix_mul_backprop(io->ln_out1, attn->attn_weight, dqkv, io->dln_out1, dAtt->dattn_weight, dAtt->dattn_bias, T, 3 * C, C, B);
}

// soft_max = log(exp(logits)/sum(exp(logits))) = logits - log(sum(exp(logits)))
void soft_max(float *logits, float *out, int B, int T, int C)
{
    for (int i = 0; i < B * T; i++)
    {
...
float cross_entropy(float *logits, int *targets, int B, int T, int C)
{
    float loss = 0.0f;
    for (int i = 0; i < B * T; i++)
    {
...
void gpt_forward(struct NetworkConfig *config, struct Gpt *model, struct Input_Output *io[], struct LayerNormContext *f)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        causal_attention(io[i]->ln_out1, io[i]->att_out, model->decoder_layer[i]->causal_attention, io[i], B, T, C, N);

        if (i == 0)
        {
            residual_layer(io[i]->att_out, f->emb_output, io[i]->res_out1, B * T * C);
        }
        else
        {
            residual_layer(io[i]->att_out, io[i - 1]->res_out2, io[i]->res_out1, B * T * C);
        }
        ...
        residual_layer(io[i]->mlp_l2_out, io[i]->ln_out2, io[i]->res_out2, B * T * C);
    }
    // final layer
...
void gpt_backward(struct NetworkConfig *config, struct Gpt *model, struct dGpt *dmodel, struct Input_Output *io[], struct LayerNormContext *f)
{
    ...
    int n = config->n_layers;
    // final layer
    matrix_mul_backprop(f->fln_output, model->final_layer->final_weight, f->dfl_output, f->dfln_output, dmodel->dfinal_layer->dfinal_weight, dmodel->dfinal_layer->dfinal_bias, T, C, V, B);
...
void adam_optimizer(float *param, float *gradients, size_t param_size, AdamState *state, float lr, float beta1, float beta2, float epsilon)
{
    state->t += 1; // Increase time step
    ...
    for (int i = 0; i < param_size; i++)
    {
        ...
        float m_hat = state->m[i] / (1.0 - pow(beta1, state->t));

        // Compute bias-corrected second moment estimate
        float v_hat = state->v[i] / (1.0 - pow(beta2, state->t));
...
void init_adam_state(AdamState *state, size_t param_size)
{
    state->m = (float *)calloc(param_size, sizeof(float));
    state->v = (float *)calloc(param_size, sizeof(float));
    state->t = 0;
}

// Free the Adam state memory
...
void update_params(float *param, float *gradients, size_t param_size, float lr)
{
    ...
    init_adam_state(&adam_state, param_size);
    adam_optimizer(param, gradients, param_size, &adam_state, lr, 0.9F, 0.999F, 1e-8);
}
...
void next_chunck(int *input, int *tokens, int *targets, int curr_pointer, int num_indices, int B, int T)
{
...
int *get_cache(const char *filename, int *num_indices)
{
...
int main()
{
    ...
    /**
     * @param batch_size No. of batches go through 4 sequence simultaneously
     * @param seq_len Each sequence have 32 tokens
     * @param num_heads Model splits the input into num_heads different smaller inputs 
     * @param d_model Dimension of the models input
     * @param vocab_size Length of token, "Hello World" = 11 if padding true
     * @param n_layers
     */

    struct NetworkConfig config = {4, 32, 8, 128, 10, 8};
    ...
    int num_indices = 0;
    int *tokens = get_cache("output.txt", &num_indices);
    printf("Tokens %d", num_indices);
    // training loop
    int steps = 1000;
    for (int i = 0; i < steps; i++)
    {
        ...
        next_chunck(f->input, tokens, targets, curr_pointer, num_indices, B, T);
        curr_pointer += total_batch;
        gpt_forward(&config, model, io, f);
        float loss = cross_entropy(f->fl_output, f->input, B, T, V);
