...
// 3. Input_Output containing intermediate inputs, outputs, etc of each decoder layer

// 4. LayerNormContext containing remaining intermediate inputs, outputs, etc of embedding and final layer
...
void map_ip_op_array(struct Input_Output* model[], const char* filename, int b, int t, int c, int n, int N) {
    size_t total_size = n * (
    ...
    for (int i = 0; i < n; i++) {
        model[i] = (struct Input_Output*)malloc(sizeof(struct Input_Output));
...
void unmap_ip_op_array(struct Input_Output* model[], int b, int t, int c, int n) {
    for (int i = 0; i < b * n; i++) {
        free(model[i]);
    }
...
void map_foo(struct LayerNormContext* model, const char* filename, int b, int t, int c) {
    size_t total_size = b * (
        sizeof(int) * t +              // input
        sizeof(float) * t * c +        // emb_output
        sizeof(float) * t * c +        // fln_output
...
void unmap_foo(struct LayerNormContext* model, int b, int t, int c) {
    // Unmap file
